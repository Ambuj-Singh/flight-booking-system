...
const Ticket=({card})=>{
    ...
    const  {pname,birth_date, booking_date,total_passenger,amount,flight,card1} = card;
    const {name,birthDate,bookingDate,totalPassenger} = card1;
    const {id,passengers,fname,source, destination,total,avaliable,price}=flight;
    ...
    return(
        <div className="flex min-h-screen flex-col items-center p-0.5 rounded-md bg-white">
        <div className="m-auto bg-white rounded-md">
            <div className="flex flex-row space-x-4">
            <Icon icon='mdi-check-outline' color="green" className="text-lg mt-2"></Icon>
        <h1 className="text-2xl">Your Ticket has been Confirmed</h1>

            </div>
        <div className="flex flex-col w-full shadow-lg mt-10 bg-white ">
            
            <div className="flex flex-row bg-blue-800 h-14 items-center rounded-t-md p-2 "> 
                ...
                <div className="flex flex-col  ">
                    ...
                    <h6 className="text-white font-medium mb-5 pb-4 w-24">{fname}#0{id}</h6>
                </div>
            </div>
